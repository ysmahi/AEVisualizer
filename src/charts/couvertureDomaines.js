/* Visuel Couverture de domaines pour AEVisualizer
* Created by Yazid Smahi on the 08/20/18 */

/* A modifier si jamais la forme du fichier change */
/* Noms des dimensions de mon fichier (fichier Excel par exemple) */
let nomDimensionLignes = 'Réseau'
let nomDimensionColonnes = 'Sous-domaines'
let nomDimensionNomDesElements = 'SA'
// let nomDimensionCouleur = 'SA'

/* Noms des valeurs que l'on veut utiliser par dimension */
let nomColonnes = ['Distribution Management', 'Performance Management', 'Distribution Accounting']

/*  A modifier si jamais on veut modifier des éléments graphiques de mon visuel */
/* Options graphiques de mon visuel */
let dispositionDesElements = 'Horizontalement et verticalement' // Peut être aussi 'Eléments courts' pour ne pas avoir de superposition
let rawWidth = 900 // Largeur du visuel
let rawHeight = 600 // Hauteur du visuel
...
/* Map function */
let nameDimensions = {
  nameDimNameElements: nomDimensionNomDesElements,
  nameColumnsRaw: nomDimensionColonnes,
  nameDimRowsRaw: nomDimensionLignes,
...
export function mapData (data) {
  return data.map((el, i) => {
    let elementData = {
      dimRow: el[nameDimensions.nameDimRowsRaw],
      dimColumn: el[nameDimensions.nameColumnsRaw],
      dimElementInside: el[nameDimensions.nameDimNameElements],
      // dimColorElements: el[nameDimensions.nameDimColorElements]
    }
...
/* Drawing function */
export function drawChart (idSelection, dataRaw) {
  ...
  let directionElements = (dispositionDesElements === 'Eléments courts')
  // let columnsName = allColumns.filter((col, indexColumn) => checkboxesColumnsValues[indexColumn])
  let columnsName = nomColonnes
